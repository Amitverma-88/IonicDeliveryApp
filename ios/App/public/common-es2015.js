...
let DatabaseService = class DatabaseService {
    constructor() {
        //promise handle
        this.returnPromise = (isSuccess, data, docRef) => ({
            isSuccess,
            data,
            docRef
        });
    ...
    checkEmailOrNumberExistGCP(object) {
        return Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"])(this, void 0, void 0, function* () {
            try {
                let db = this.db;
                let querySnapshot = null;
                if (object.type != "email") {
                    querySnapshot = yield this.db.collection("UserCollection").where("phoneNumber", "==", object.countryCode + Number(object.data)).get();
                }
                else {
                    querySnapshot = yield this.db.collection("UserCollection").where("email", "==", object.data.toLowerCase()).get();
                }
    ...
    //get user data by id
    getUserDataByPhoneNo(phoneNumber) {
        return Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"])(this, void 0, void 0, function* () {
            try {
                const dataArray = [];
                let docRef = this.db.collection('UserCollection');
                let querySnapshot = yield docRef.where("phoneNumber", "==", phoneNumber).get();
                if (!querySnapshot.empty) {
